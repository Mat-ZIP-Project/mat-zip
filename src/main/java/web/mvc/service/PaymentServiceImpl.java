...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PreparationResDto prepareValid(User user, PreparationReqDto request) throws BasicException, IamportResponseException, IOException {
        ...
        // ✅ 보안 검증: 예약의 실제 소유자 ID와 현재 로그인된 사용자 ID가 일치하는지 확인
        if (!reservation.getUser().getId().equals(user.getId())) { // User 엔티티의 ID 필드가 'id'라고 가정
            log.error("사전 검증 권한 없음: 요청 사용자 ID '{}'가 예약 ID '{}'의 소유자가 아닙니다. (예약 소유자 ID: {})",
                    user.getId(), reservationId, reservation.getUser().getId());
            throw new BasicException(ErrorCode.RESERVATION_NOT_FOUND); // UNAUTHORIZED_ACCESS 에러코드 필요
        }
     ...
     */
    @Override
    public Payment cancelPayment(String impUid, BigDecimal amount, String reason) throws BasicException, IamportResponseException, IOException {
        log.warn("결제 취소 요청 시작 - impUid: {}, 금액: {}, 사유: {}", impUid, amount, reason);

        // 1. 아임포트에서 해당 결제 정보 조회 (유효성 검증)
        IamportResponse<Payment> iamportResponse = client.paymentByImpUid(impUid);
        if (iamportResponse.getCode() != 0 || iamportResponse.getResponse() == null) {
            log.error("아임포트 결제 정보 조회 실패 (취소 대상): {}", iamportResponse.getMessage());
            throw new BasicException(ErrorCode.PAYMENT_INFO_NOT_FOUND);
        }
        Payment targetPayment = iamportResponse.getResponse();

        // 2. DB에서 해당 impUid에 해당하는 결제 정보를 찾습니다.
        ReservationPayment localPayment = reservationPaymentRepository.findByImpUid(impUid)
                .orElseThrow(() -> {
                    log.error("DB에 impUid {} 에 해당하는 Payment 정보가 없음.", impUid);
                    return new BasicException(ErrorCode.NOTFOUNT_MERCHANTUID);
                });
        ...
        // 4. 아임포트 결제 취소 api 호출을 위한 데이터 준비
        ...
        log.info("결제 취소 최종 성공: impUid={}", impUid);
