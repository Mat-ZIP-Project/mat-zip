...
//    public ResponseEntity<?> registerToken(@RequestBody FcmTokenReqDto request) {
//        try {
//            User currentUser = userRepository.findById(1L)
//                    .orElseThrow(() -> new BasicException(ErrorCode.USER_INFO_MISSING));
//            fcmService.registerFcmToken(currentUser, request.getDeviceToken());
//            return ResponseEntity.ok().build();
//        } catch (BasicException e) {
//            if (e.getErrorCode() == ErrorCode.USER_INFO_MISSING) {
...
//    public ResponseEntity<?> sendTestNotification(@PathVariable Long userId, @RequestBody NotificationResDto payload) {
//        try {
//            User targetUser = userRepository.findById(userId)
//                    .orElseThrow(() -> new BasicException(ErrorCode.USER_INFO_MISSING));
//
//            log.info("테스트 알림 전송 요청: 사용자 ID='{}', 제목='{}', 내용='{}'", userId, payload.getTitle(), payload.getBody());
